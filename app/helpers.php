...
function money(float|string|int $v): string { return 'R$ '.number_format((float)$v,2,',','.'); }
...
function random_code(string $prefix='AQV'): string { return $prefix.'-'.strtoupper(bin2hex(random_bytes(5))); }
...
function require_api_key(): void {
    ...
    if (!$expected || !$provided || !hash_equals($expected,$provided)) json_response(['ok'=>false,'error'=>'unauthorized'],401);
}
...
function normalize_document(string $v): string { return strtoupper(preg_replace('/[^A-Za-z0-9]/','',$v) ?? ''); }
