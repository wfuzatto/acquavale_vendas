...
final class OrderService {
    public function create(array $payload,array $photos): array {
        ...
        $products=[];
        foreach($s->fetchAll() as $p) $products[(int)$p['id']]=$p;
        ...
        if(!is_array($visitors) || count($visitors)!==$ticketUnits) {
            throw new RuntimeException('Preencha os dados de todos os visitantes.');
        }
        ...
        try{
            ...
            foreach($visitors as $i=>$v){
                ...
                if(!isset($products[$productId]) || (int)$products[$productId]['requires_visitor']!==1) {
                    throw new RuntimeException('Produto de ingresso inválido no visitante.');
                }
                $p=$products[$productId];

                $first=trim((string)($v['first_name']??''));
                $last=trim((string)($v['last_name']??''));
                $email=trim((string)($v['email']??$buyerEmail));
                $phone=trim((string)($v['phone']??$buyerPhone));
                $entry=trim((string)($v['entry_date']??''));
                $docType=strtoupper(trim((string)($v['document_type']??'CPF')));
                $doc=\normalize_document((string)($v['document_number']??''));
                $sex=strtolower(trim((string)($v['sex']??'nao_informado')));
                $consent=(string)($v['biometric_consent']??'')==='1';

                if($first===''||$last===''||!filter_var($email,FILTER_VALIDATE_EMAIL)||$phone===''||$doc==='') {
                    throw new RuntimeException('Dados incompletos de um visitante.');
                }
                ...
                $photoPath=$this->storePhoto($photos,$i);
                $saved[]=STORAGE_ROOT.'/private/visitors/'.$photoPath;
    ...
    private function storePhoto(array $photos,int $index): string {
        if(!isset($photos['tmp_name'][$index]) || !is_uploaded_file($photos['tmp_name'][$index])) {
            throw new RuntimeException('Foto obrigatória para cada visitante.');
        }
        if(($photos['error'][$index]??UPLOAD_ERR_NO_FILE)!==UPLOAD_ERR_OK) {
            throw new RuntimeException('Falha no envio de uma foto.');
        }

        $max=((int)\cfg('uploads.max_photo_mb',8))*1024*1024;
        ...
        $dest=STORAGE_ROOT.'/private/visitors/'.$name;
        if(!move_uploaded_file($photos['tmp_name'][$index],$dest)) {
    ...
    public function simulatePayment(int $orderId): void {
        $pdo=\db();
        $pdo->beginTransaction();
        $s=$pdo->prepare("SELECT status FROM orders WHERE id=? FOR UPDATE");
        $s->execute([$orderId]);
        $o=$s->fetch();
        if(!$o){
            $pdo->rollBack();
    ...
    public function getOrderByCode(string $code): ?array {
        $s=\db()->prepare("SELECT * FROM orders WHERE order_code=?");
        $s->execute([$code]);
        $o=$s->fetch();
        if(!$o) return null;
